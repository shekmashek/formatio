...
class CollaborationController extends Controller
{
    ...
    public function create_cfp_etp(Request $req)
    {
        ...
        if(Gate::allows('isInvite') || Gate::allows('isPending')) return back()->with('error', "Vous devez faire un abonnement avant de faire une collaboration");
          ...
          $verify = $verify1->id;
                if ($verify <= 0) {
                    $msg = $this->collaboration->verify_collaboration_cfp_etp($responsable_cfp->cfp_id, $responsable->entreprise_id, $req->nom_format,$responsable_cfp->id);
            ...
            //    $verify1 = $this->fonct->verifyGenerique("demmande_cfp_etp", ["demmandeur_cfp_id", "inviter_etp_id"], [$responsable_cfp->cfp_id, $entreprise_id]);
               ...
               $verify = $verify2->id;
                ...
                if ($verify <= 0) {
                    ...
                    $msg = $this->collaboration->verify_collaboration_etp_cfp($responsable_cfp->cfp_id, $entreprise_id, $req->nom_cfp,$responsable_etp);
...
*/
    ...
    {
        $user_id = Auth::user()->id;
        $resp_etp_id = $this->fonct->findWhereMulitOne("responsables", ["user_id"], [$user_id])->id;
        return $this->collaboration->accept_invitation_collaboration_etp_cfp($id,$resp_etp_id);
    }

    public function accept_invitation_cfp_etp($id)
    {
        $user_id = Auth::user()->id;
        $resp_cfp_id = $this->fonct->findWhereMulitOne("responsables_cfp", ["user_id"], [$user_id])->id;
        return $this->collaboration->accept_invitation_collaboration_cfp_etp($id,$resp_cfp_id);
    }
