...
class AdminController extends Controller
{
    ...
    public function profile_resp()
    {
        ...
        if (Gate::allows('isChefDeService')) {
            $user = $fonct->findWhereMulitOne(("employers"),["user_id"],[$id_user])->photos;
            $photo ='';
            if($user == null){
                $user = DB::select('select SUBSTRING(nom_emp, 1, 1) AS nm,  SUBSTRING(prenom_emp, 1, 1) AS pr from employers where user_id = ?', [$id_user]);
                $photo = 'non';
                //  $user = 'users/users.png';
            } else{
                $user = 'images/employes/' . $user;
        ...
        if (Gate::allows('isCFP')) {
            ...
            $cfp_id =  $fonct->findWhereMulitOne("responsables_cfp",["user_id"],[$id_user])->cfp_id;
            $invitation_etp = DB::select('select id_etp,nom_resp,prenom_resp,email_resp,nom_etp,nom_secteur from v_collab_cfp_etp where cfp_id = ? and statut = ? and demmandeur = ?',[$cfp_id,1,'etp']);
