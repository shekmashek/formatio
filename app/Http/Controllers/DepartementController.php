...
*/
    ...
    public function ajouter_chef_de_service(Request $request){
        ...
        if($request->chef_de_service != "null"){
            ...
            DB::update('update role_users set activiter = 0 where user_id = ? and role_id != 8', [$emp_info->user_id]);
            DB::insert('insert into role_users (user_id,role_id,prioriter,activiter) values (?,8,0,1)', [$emp_info->user_id]);
            return back();
        }
        else{
    ...
    public function modifier_chef_de_service(Request $request){
        ...
        if($request->chef_de_service != "null"){
            ...
            DB::delete('delete from role_users where user_id = ? and role_id = ?', [$request->ancien_user_chef,8]);

            /**ajout  nouveau chef*/
            DB::insert('insert into chef_de_service_entreprises (service_id, chef_de_service_id) values (?, ?)', [$request->service_id, $request->chef_de_service]);
            $emp_info = $fonct->findWhereMulitOne("employers",["id"],[$request->chef_de_service]);

            DB::update('update role_users set activiter = 0 where user_id = ? and role_id != ?', [$emp_info->user_id,8]);
            DB::insert('insert into role_users (user_id,role_id,prioriter,activiter) values (?,?,?,?)', [$emp_info->user_id,8,0,1]);
            return back();
        }
        else{
            return back()->with('erreur_manager',"Choisissez un chef de service avant d'enregistrer");
