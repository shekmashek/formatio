...
create or replace view v_nombre_detail_groupe_interne as
select
    groupe_interne_id as groupe_id,
    count(id) as nombre_detail
from details_interne group by groupe_id;

create or replace view v_rapport_presence_interne as
select
    e.groupe_id,
    stagiaire_id,
    matricule,
    ...
    n.nombre_detail - sum(e.statut_presence)  as nombre_presence
from v_emargement_interne e join v_nombre_detail_groupe_interne n on e.groupe_id = n.groupe_id
group by 
    e.groupe_id,
    stagiaire_id,
    matricule,
