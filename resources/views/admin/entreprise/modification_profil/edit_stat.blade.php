...
<center>
    @if (\Session::has('erreur_stat'))
        <div class="alert alert-danger col-md-4">
            <ul>
                <li>{!! \Session::get('erreur_stat') !!}</li>
            </ul>
        </div>
    @endif
    <div class="col-lg-4">
        <div class="p-3 form-control">
            <p style="text-align: left">Modifier le STAT de l'entreprise</p>
            <form   class="btn-submit" action="{{route('enregistrer_stat_entreprise',$etp->id)}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="row px-3 mt-4">
                    <div class="form-group mt-1 mb-1">
                    <input type="text" value="   {{ $etp->stat}}" class="form-control test input"  name="stat">
                    </div>
                </div>
