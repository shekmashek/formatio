...
<style>
        ...
        @can('isReferent')
            <section class="section_recherche m-0 p-2">
                <div class="d-flex py-1 align-items-center align-content-center">
                    ...
                    <input type="text" id="matricule_search" data-id="{{ $etp_id }}" name="matricule_stg" placeholder="Entrez le matricule ou le nom ici . . ." class="matricule_search_input form-control">
                    <input type="hidden" id="id_entreprise" value="{{ $etp_id }}">
    ...
    <script>
        ...
        $(".rechercher").on('click',function(e){
            ...
            var etp_id = @php echo $etp_id; @endphp;
            var groupe_id = @php echo $projet[0]->groupe_id; @endphp;
            $.ajax({
                type: "GET",
                url: "{{route('one_stagiaire_interne')}}",
                data:{
                    Id:id,
                    etp:etp_id,
                    groupe:groupe_id
        ...
        $(".span_ajout").on('click',function(e){
            ...
            $.ajax({
                type: "GET",
                url: "{{route('add_participant_groupe_interne')}}",
                data:{
                    Id:id,
                    groupe:groupe_id
                },
