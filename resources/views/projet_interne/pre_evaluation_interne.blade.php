<div class="row d-flex text-center">
    ...
    @if ($evaluation_avant == 0 || $evaluation_avant == null)
        <form action="{{ route('insert_evaluation_stagiaire_interne') }}" method="POST">
        @csrf
    ...
    @else
        <form action="{{ route('modifier_evaluation_stagiaire_interne') }}" method="POST">
            @csrf
            <input type="hidden" name="groupe" value="{{ $projet[0]->groupe_id }}">
            <input type="hidden" name="module" value="{{ $projet[0]->module_id }}">
            <div class="col-md-12 d-flex justify-content-around">
                            ...
                            <tr>
                                ...
                                @for ($i = 0; $i < count($competences); $i++)
                                    @foreach ($evaluation_stg as $e_stg)
                                        @if ($e_stg->stagiaire_id == $stg->stagiaire_id && $e_stg->competence_a_evaluers_interne_id == $competences[$i]->id)
